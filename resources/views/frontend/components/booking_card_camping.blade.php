<div class="bg-white border border-gray-200 rounded-2xl p-6 ">
    <div class="mb-6">
        <span class="text-3xl font-extrabold text-gray-900">Rp{{ number_format($package->price, 0, ',', '.') }}</span>
        <span class="text-base font-normal text-gray-500"> / paket</span>
    </div>
    ...
    <div class="bg-white border border-gray-300 rounded-lg overflow-hidden mb-6">
        <div class="p-3 border-b border-gray-300 flex justify-between items-center bg-gray-50">
            <span class="text-xs font-bold text-gray-700 uppercase tracking-wide">Status Paket</span>
            <span class="text-sm font-medium {{ $package->status == 'Available' ? 'text-green-600' : 'text-red-600' }}">
                {{ $package->status }}
            </span>
        </div>
        <div class="p-4 bg-white">
            <p class="text-sm text-gray-600">Pemesanan paket camping dilakukan secara langsung melalui WhatsApp untuk memastikan ketersediaan jadwal dan perlengkapan.</p>
        </div>
    </div>

    <a href="https://wa.me/6287834443012?text={{ urlencode('Halo Admin Browky, saya tertarik dengan ' . $package->name . ' seharga Rp' . number_format($package->price, 0, ',', '.') . '. Mohon informasi lebih lanjut.') }}" target="_blank" class="w-full flex justify-center items-center gap-2 py-3.5 px-4 rounded-lg text-sm font-medium text-white bg-primary hover:bg-blue-950 transition-colors shadow-md group">
