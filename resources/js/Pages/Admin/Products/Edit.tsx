...
export default function Edit({ product }: ProductProps) {
  const { data, setData, post, processing, errors, reset } = useForm({
    ...
    stock: product.stock,
    special_badge: product.special_badge || '',
    description: product.description,
    specifications: product.specifications || '',
    cover_image: null as File | null,
  });

  const currentImgSrc = product.cover_image 
    ? (product.cover_image.startsWith('http') ? product.cover_image : `/storage/${product.cover_image}`)
    : null;

  const [preview, setPreview] = useState<string | null>(currentImgSrc);
  ...
  const handleReset = () => {
    reset();
    setPreview(currentImgSrc);
    toast.info("Perubahan form telah di-reset.");
  };

  const handleSaveDraft = () => {
    toast.success("Draf produk berhasil disimpan!");
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    post(`/admin/products/${product.id}`, {
      ...
      onError: () => {
        toast.error("Gagal memperbarui produk. Mohon periksa kembali form input Anda.");
      }
    });
  };
    ...
    <AdminLayout title="Edit Produk">
      <div className="pb-16 sm:pb-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold tracking-tight text-foreground">Edit Produk "{product.name}"</h1>
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">
              Perbarui informasi spesifikasi dan tarif sewa peralatan.
            </p>
          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <Field>
                  ...
                  />
                  <FieldDescription>Perbarui informasi detail teknis peralatan jika ada perubahan.</FieldDescription>
                </Field>
              </CardContent>
            </Card>
          </div>
