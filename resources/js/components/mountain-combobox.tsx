...
export function MountainCombobox({ value, onChange, error, initialList }: MountainComboboxProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');
  const [customInputMode, setCustomInputMode] = useState(false);
  const [mountains, setMountains] = useState<string[]>(() => {
    const combined = Array.from(new Set([...(initialList || []), ...DEFAULT_MOUNTAINS]));
  ...
  const handleSelect = (mountainName: string) => {
    onChange(mountainName);
    setOpen(false);
    setSearch('');
    setCustomInputMode(false);
  };

  const handleDeleteItem = (e: React.MouseEvent, mName: string) => {
    e.stopPropagation();
    setMountains((prev) => prev.filter((item) => item !== mName));
    if (value === mName) {
      onChange('');
    }
  };

  const handleAddCustom = (newMountain: string) => {
    const trimmed = newMountain.trim();
    if (!trimmed) return;
    if (!mountains.includes(trimmed)) {
      setMountains((prev) => [...prev, trimmed]);
    }
    onChange(trimmed);
    setOpen(false);
    setSearch('');
    ...
    <div className="space-y-1">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">{value || "Pilih atau ketik gunung..."}</span>
          ...
          {!customInputMode ? (
            <div className="space-y-2">
              <div className="relative flex items-center">
                ...
                <Input
                  placeholder="Cari atau ketik gunung baru..."
                  value={search}
                  ...
                  filteredMountains.map((m) => (
                    <div
                      ...
                      className={cn(
                        ...
                        value === m && "bg-zinc-100 dark:bg-zinc-800 font-medium text-foreground"
                      )}
                    >
                      <div className="flex items-center gap-2 truncate pr-2">
                        {value === m && <Check className="h-3.5 w-3.5 text-primary shrink-0" />}
                        <span className="truncate">{m}</span>
                      </div>
                      <button
                        type="button"
                        onClick={(e) => handleDeleteItem(e, m)}
                        title={`Hapus "${m}" dari daftar`}
              ...
              <div className="border-t border-border pt-1">
                {search.trim() && !mountains.some(m => m.toLowerCase() === search.trim().toLowerCase()) ? (
                  <button
                    type="button"
                  ...
                  >
                    <Plus className="h-3.5 w-3.5" />
                    <span>Ketik gunung baru secara manual...</span>
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2 p-1">
              <p className="text-xs font-medium text-foreground">Input Gunung Baru</p>
