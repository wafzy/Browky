...
<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-8">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Edit Paket Porter</h1>
        <p class="text-xs text-zinc-500 mt-1">{{ $porter->name }}</p>
    </div>
...
<form action="{{ route('admin.porters.update', $porter) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="bg-white border border-zinc-200/80 rounded-xl shadow-xs p-6">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $porter->name) }}" placeholder="Contoh: Paket Porter 1 Hari" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div>
                            ...
                            <select id="category" name="category" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                                <option value="Tektok" {{ old('category', $porter->category) == 'Tektok' ? 'selected' : '' }}>Tektok</option>
                                <option value="Porter Inap" {{ old('category', $porter->category) == 'Porter Inap' ? 'selected' : '' }}>Porter Inap</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="status" name="status" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                                <option value="Available" {{ old('status', $porter->status) == 'Available' ? 'selected' : '' }}>Available</option>
                                <option value="Unavailable" {{ old('status', $porter->status) == 'Unavailable' ? 'selected' : '' }}>Unavailable</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="special_badge" name="special_badge" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition">
                                ...
                                <option value="Paling Diminati" {{ old('special_badge', $porter->special_badge) == 'Paling Diminati' ? 'selected' : '' }}>Paling Diminati</option>
                                <option value="Pilihan Lokal" {{ old('special_badge', $porter->special_badge) == 'Pilihan Lokal' ? 'selected' : '' }}>Pilihan Lokal</option>
                                <option value="Spesial Diskon" {{ old('special_badge', $porter->special_badge) == 'Spesial Diskon' ? 'selected' : '' }}>Spesial Diskon</option>
                                <option value="Paling Hemat" {{ old('special_badge', $porter->special_badge) == 'Paling Hemat' ? 'selected' : '' }}>Paling Hemat</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="number" id="price_per_day" name="price_per_day" value="{{ old('price_per_day', $porter->price_per_day) }}" placeholder="150000" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                        </div>
                        <div>
                            <label for="mountain" class="block text-xs font-semibold text-zinc-700 mb-1.5">Gunung <span class="text-red-500">*</span></label>
                            <input type="text" id="mountain" name="mountain" value="{{ old('mountain', $porter->mountain) }}" placeholder="Contoh: Gunung Sumbing" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                        </div>
                    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-zinc-200/80 rounded-xl shadow-xs p-6">
                ...
                <div class="mb-4">
                    <p class="text-xs font-semibold text-zinc-600 mb-2">Foto saat ini</p>
                    <div class="aspect-square w-full rounded-lg overflow-hidden border border-zinc-200">
                        <img src="{{ $imgSrc }}" alt="{{ $porter->name }}" class="w-full h-full object-cover">
                    </div>
                </div>
                @endif
                ...
                <label for="porterImageInput" class="flex flex-col items-center justify-center gap-2 w-full h-24 rounded-lg border-2 border-dashed border-zinc-300 cursor-pointer hover:border-zinc-900 hover:bg-zinc-50/50 transition">
                    ...
                    <span class="text-xs font-semibold text-zinc-600">{{ $porter->image ? 'Ganti foto' : 'Pilih foto' }}</span>
                </label>
                <input type="file" name="image" id="porterImageInput" accept="image/*" class="hidden">
                @if($porter->image)
                <p class="text-[10px] text-zinc-400 mt-2 text-center">Biarkan kosong jika tidak ingin mengubah foto.</p>
                @endif
            </div>
