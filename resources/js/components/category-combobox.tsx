...
const DEFAULT_CATEGORIES = [
  ...
  'Alat Masak & Kompor',
  'Lampu & Penerangan',
  'Sepatu & Tracking Pole',
  'Matras & Air Bed',
  'Jaket & Pakaian Outdoor',
  'Aksesoris Camping',
];
...
export function CategoryCombobox({ value, onChange, error }: CategoryComboboxProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState('');
  const [customInputMode, setCustomInputMode] = useState(false);
  const [categories, setCategories] = useState<string[]>(() => {
    if (value && !DEFAULT_CATEGORIES.includes(value)) {
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-2 shadow-md rounded-md" align="start" sideOffset={4}>
          {!customInputMode ? (
                  ...
                  filteredCategories.map((cat) => (
                    <div
                      ...
                      className={cn(
                        ...
                        value === cat && "bg-zinc-100 dark:bg-zinc-800 font-medium text-foreground"
                      )}
                    >
                      ...
                      <button
                        type="button"
                        onClick={(e) => handleDeleteItem(e, cat)}
                        title={`Hapus "${cat}" dari daftar`}
            ...
            <div className="space-y-2 p-1">
              <p className="text-xs font-medium text-foreground">Input Kategori Baru</p>
