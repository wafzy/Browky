...
<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-8">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Edit Produk</h1>
        <p class="text-xs text-zinc-500 mt-1">{{ $product->name }}</p>
    </div>
...
<form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="bg-white border border-zinc-200/80 rounded-xl shadow-xs p-6">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" placeholder="Contoh: Tenda Dome 4 Person" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input list="productCategories" id="category" name="category" value="{{ old('category', $product->category) }}" placeholder="Pilih atau ketik kategori" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                        ...
                        <div>
                            ...
                            <select id="special_badge" name="special_badge" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition">
                                ...
                                <option value="Paling Diminati" {{ old('special_badge', $product->special_badge) == 'Paling Diminati' ? 'selected' : '' }}>Paling Diminati</option>
                                <option value="Pilihan Lokal" {{ old('special_badge', $product->special_badge) == 'Pilihan Lokal' ? 'selected' : '' }}>Pilihan Lokal</option>
                                <option value="Spesial Diskon" {{ old('special_badge', $product->special_badge) == 'Spesial Diskon' ? 'selected' : '' }}>Spesial Diskon</option>
                                <option value="Paling Hemat" {{ old('special_badge', $product->special_badge) == 'Paling Hemat' ? 'selected' : '' }}>Paling Hemat</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="number" id="price_per_day" name="price_per_day" value="{{ old('price_per_day', $product->price_per_day) }}" placeholder="50000" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                        </div>
                        <div>
                            <label for="stock" class="block text-xs font-semibold text-zinc-700 mb-1.5">Stok <span class="text-red-500">*</span></label>
                            <input type="number" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" min="0" class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition" required>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="specifications" name="specifications" rows="3" placeholder="Spesifikasi teknis produk..." class="w-full rounded-lg border border-zinc-300 bg-white py-2.5 px-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-900 transition resize-none">{{ old('specifications', $product->specifications) }}</textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            {{-- Cover Image Card --}}
            <div class="bg-white border border-zinc-200/80 rounded-xl shadow-xs p-6">
                <h2 class="text-sm font-bold text-zinc-900 mb-4 pb-4 border-b border-zinc-100">Cover Image</h2>

                @if($product->cover_image)
                <div class="mb-4">
                    <p class="text-xs font-semibold text-zinc-600 mb-2">Cover saat ini</p>
                    <div class="aspect-square w-full rounded-lg overflow-hidden border border-zinc-200">
                        <img src="{{ str_starts_with($product->cover_image, 'http') ? $product->cover_image : asset('storage/' . $product->cover_image) }}" alt="Cover Image" class="w-full h-full object-cover">
                    </div>
                </div>
                @endif

                <div id="coverImagePreviewContainer" class="hidden mb-4">
                    <p class="text-xs font-semibold text-zinc-600 mb-2">Preview baru</p>
                ...
                <label for="coverImageInput" class="flex flex-col items-center justify-center gap-2 w-full h-24 rounded-lg border-2 border-dashed border-zinc-300 cursor-pointer hover:border-zinc-900 hover:bg-zinc-50/50 transition">
                    <svg class="h-6 w-6 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5"/></svg>
                    <span class="text-xs font-semibold text-zinc-600">{{ $product->cover_image ? 'Ganti cover' : 'Pilih cover' }}</span>
                </label>
                <input type="file" name="cover_image" id="coverImageInput" accept="image/*" class="hidden">
                @if($product->cover_image)
                <p class="text-[10px] text-zinc-400 mt-2 text-center">Biarkan kosong jika tidak ingin mengubah cover.</p>
                @endif
            </div>

            {{-- Submit Actions --}}
            <div class="bg-white border border-zinc-200/80 rounded-xl shadow-xs p-6">
                <h2 class="text-sm font-bold text-zinc-900 mb-4">Simpan Perubahan</h2>
...
{{-- Generic Delete Form for Images --}}
